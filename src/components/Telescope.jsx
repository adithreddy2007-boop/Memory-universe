...
export default function Telescope({ shareCode, universeId, isPreview, onExit }) {
  ...
  useEffect(() => {
    ...
    // ---- Full-coverage nebula skydome ----
    // Instead of a thin band on black, this paints a dense equirectangular
    // cloud texture covering the ENTIRE sphere around the viewer, so no
    // matter which direction you drag to look, the theme's color saturates
    // the whole view — like the real astrophotography reference images,
    // not scattered dots on a mostly-black background.
    function skydomeTexture() {
      const w = 2048, h = 1024;
      const cnv = document.createElement('canvas'); cnv.width = w; cnv.height = h;
      const ctx = cnv.getContext('2d');
      const [c1, c2] = theme.nebula;
      const bandColor = theme.bandColor || c1;

      // base wash: never leaves flat black, always tinted with the theme
      const base = ctx.createLinearGradient(0, 0, 0, h);
      base.addColorStop(0, theme.bg);
      base.addColorStop(0.5, c2);
      base.addColorStop(1, theme.bg);
      ctx.fillStyle = base;
      ctx.fillRect(0, 0, w, h);

      // large soft color washes across the ENTIRE canvas (full coverage,
      // not a strip) — this is what makes "blue everywhere" instead of
      // "a little blue, mostly black"
      const washColors = [c1, c2, bandColor, theme.glow];
      for (let i = 0; i < 34; i++) {
        const x = rng() * w;
        const y = rng() * h;
        const r = 260 + rng() * 480;
        const col = washColors[Math.floor(rng() * washColors.length)];
        const alpha = 0.22 + rng() * 0.28;
        const g = ctx.createRadialGradient(x, y, 0, x, y, r);
        g.addColorStop(0, col + Math.floor(alpha * 255).toString(16).padStart(2, '0'));
        g.addColorStop(1, 'rgba(0,0,0,0)');
        ctx.fillStyle = g;
        ctx.fillRect(0, 0, w, h);
        // wrap-around coverage at the seams so it tiles seamlessly left/right
        if (x < r) { ctx.fillStyle = g; ctx.save(); ctx.translate(w, 0); ctx.fillRect(-w, 0, w, h); ctx.restore(); }
        if (x > w - r) { ctx.save(); ctx.translate(-w, 0); ctx.fillRect(0, 0, w, h); ctx.restore(); }
      }

      // turbulent filament texture — finer, denser cloud detail layered on top
      for (let i = 0; i < 260; i++) {
        const x = rng() * w;
        const y = rng() * h;
        const r = 40 + rng() * 140;
      ...
      // dense fine star speckle across the whole dome for real-photo texture
      const starPalette = theme.starPalette || [[theme.star, 1]];
      for (let i = 0; i < 3500; i++) {
        const x = rng() * w;
        const y = rng() * h;
        const rSize = Math.pow(rng(), 3) * 1.6 + 0.3;
        const col = starPalette[Math.floor(rng() * starPalette.length)][0];
    ...
    const skyDomeGeo = new THREE.SphereGeometry(1400, 64, 40);
    const skyDomeMat = new THREE.MeshBasicMaterial({ map: skydomeTexture(), side: THREE.BackSide, fog: false });
